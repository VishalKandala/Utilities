...
int main(int argc, char *argv[])
{
  ...
  MPI_Allreduce(&local_median,&pivot,1,MPI_INT,MPI_SUM,sub_hypercube_comm);
	...
	if (nbr_k > my_id) {
	    ...
	    // ***** Add MPI call here *****
      MPI_Send(&list_size_gt,1,MPI_INT,nbr_k,MPI_COMM_WORLD);

	    // MPI-3: Receive number of elements less than or equal to pivot

	    // ***** Add MPI call here *****
      MPI_Recv(&nbr_list_size,1,MPI_INT,nbr_k,MPI_COMM_WORLD);
	    ...
	    // ***** Add MPI call here *****
      MPI_Send(&(list[idx]),list_size_gt,MPI_INT,nbr_k,MPI_COMM_WORLD);
	    // MPI-5: Receive neighbor's list of elements that are less than or equal to pivot
      MPI_Recv(nbr_list,nbr_list_size,MPI_INT,nbr_k,MPI_COMM_WORLD);
	    // ***** Add MPI call here *****
      ...
      MPI_Recv(&nbr_list_size,1,MPI_INT,nbr_k,MPI_COMM_WORLD);
	    ...
	    // ***** Add MPI call here *****
      MPI_Send(&list_size_leq,1,MPI_INT,nbr_k,MPI_COMM_WORLD);
	    // Allocate storage for neighbor's list
	    nbr_list = (int *) calloc(nbr_list_size, sizeof(int));
      ...
      MPI_Send(list,list_size_leq,MPI_INT,nbr_k,MPI_COMM_WORLD);
	    // ***** Add MPI call here *****
